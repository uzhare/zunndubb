...
class NextPreviousPagerNode(Node):
    ...
    def render(self, context):
        ...
        return get_template("paginator/pager.html").render(
            Context({
                'page': page,
                'centered': centered,
...
class PaginationNode(Node):
    ...
    def render(self, context):
        ...
        previous_label = str(kwargs.get("previous_label", "&lsaquo;"))
        next_label = str(kwargs.get("next_label", "&rsaquo;"))
        ...
        return get_template("paginator/pagination.html").render(
            Context({
                'page': page,
                'alignment': alignment,
...
def pager(parser, token):
    ...
    if len(bits):
        for bit in bits:
            match = kwarg_re.match(bit)
            if not match:
                raise TemplateSyntaxError("Malformed arguments to pager tag")
            name, value = match.groups()
            kwargs[name] = parser.compile_filter(value)

    return NextPreviousPagerNode(page, kwargs)
